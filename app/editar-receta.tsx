...
export default function EditarRecetaScreen() {
  ...
  const [recetaOriginal, setRecetaOriginal] = useState<any>(null);
  ...
  useEffect(() => {
    const cargar = async () => {
      try {
        let receta: any = RECETAS.find(r => r.id === id);
        let recLocal: any = null;
        
        if (receta) {
          setRecetaOriginal({ ...receta, preset: true });
        } else {
          if (user && !isGuest) {
            const docSnap = await getDoc(doc(db, 'users', user.uid, 'recipes', id as string));
            if (docSnap.exists()) recLocal = docSnap.data();
          } else {
            const data = await AsyncStorage.getItem('recetas_usuario');
            if (data) {
              const lista = JSON.parse(data);
              recLocal = lista.find((r: any) => r.id === id);
            }
          }
          if (recLocal) setRecetaOriginal(recLocal);
        }

        const target = receta || recLocal;
        if (target) {
          setNombre(target.name);
          setCategoria(target.category);
          setPoston(target.poston?.toString() || '');
          setAmasado(target.proceso?.amasado || '');
          setTempMasa(target.proceso?.tempMasa?.toString() || '25');
          setIngredients(target.ingredients.map((i: any) => ({ ...i, id: i.id || genId() })));
          setLevado((target.levado || []).map((e: any) => ({ ...e, id: e.id || genId() })));
          setCocTemp(target.coccion?.temp?.toString() || '220');
          setCocTiempo(target.coccion?.tiempo?.toString() || '30');
          setCocTempInterna(target.coccion?.tempInterna?.toString() || '93');
          setCocHorno(target.coccion?.ovenType || 'Estático');
          setCocVapor(target.coccion?.vapor || false);
          setCocNotas(target.coccion?.notas || '');
        }
      } catch (e) {
        console.error(e);
      }
  ...
  const guardar = async () => {
    ...
    const recetaEditada = {
      id: recetaOriginal?.preset ? `u_${genId()}` : recetaOriginal?.id,
    ...
    try {
      if (user && !isGuest) {
        await setDoc(doc(db, 'users', user.uid, 'recipes', recetaEditada.id), recetaEditada);
      } else {
        const data = await AsyncStorage.getItem('recetas_usuario');
        const lista = data ? JSON.parse(data) : [];
        if (recetaOriginal?.preset) {
           lista.push(recetaEditada);
        } else {
           const idx = lista.findIndex((r: any) => r.id === recetaEditada.id);
           if (idx >= 0) lista[idx] = recetaEditada;
           else lista.push(recetaEditada);
        }
        await AsyncStorage.setItem('recetas_usuario', JSON.stringify(lista));
      }
      Alert.alert('Éxito', recetaOriginal?.preset ? 'Copia guardada con éxito' : 'Receta actualizada', [
        { text: 'OK', onPress: () => router.push('/(tabs)') }
      ]);
    } catch { Alert.alert('Error', 'No se pudo guardar'); }
  };

  if (cargando) return <View style={styles.container}><Text style={styles.loadingText}>Cargando...</Text></View>;
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle} numberOfLines={1}>Editar: {nombre || 'Receta'}</Text>
      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 20 }]}>
        ...
        {section === 'info' && (
          <View>
            ...
            {recetaOriginal?.preset && (
              <View style={styles.warningBox}>
                <Text style={styles.warningText}>⚠️ Esta es una receta integrada. Al guardar se creará una copia personal con tus cambios.</Text>
              </View>
            )}
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  loadingText: { color: '#8A7F72', textAlign: 'center', marginTop: 40 },
  ...
  warningBox: { backgroundColor: 'rgba(242,184,75,0.08)', borderLeftWidth: 3, borderLeftColor: '#F2B84B', borderRadius: 8, padding: 12, marginTop: 4 },
  warningText: { color: '#F2B84B', fontSize: 12, lineHeight: 18 },
